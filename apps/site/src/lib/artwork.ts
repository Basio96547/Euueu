...
/**
 * معرِّفٌ فريد لكل نسخةٍ من الرسم في الصفحة.
 *
 * العدّاد هنا لا في واجهة المكوّن: واجهةُ المكوّن تُنفَّذ لكل نسخةٍ من
 * جديد، فـ`let seq = 0` فيها تُصفَّر في كل مرّة ويخرج المعرّف نفسه لكل
 * البطاقات. والوحدة تُحمَّل مرّةً واحدة، فالعدّاد فيها يعدّ فعلاً.
 *
 * والتجزئة من الاسم لا تكفي: المنتج الواحد قد يظهر مرّتين في الصفحة
 * (أحدثُ ما وصل، ثم المستعمل)، فيتصادم معرّفاه — و`url(#…)` يحلّ إلى
 * الأول دائماً.
 */
let artSeq = 0;
export const nextArtId = (kind: string) => `${kind}${(artSeq += 1)}`;
