...
قاعدتان تفرضهما طبيعة السوق:

**(1) العملة المعلنة في البيانات المنظمة يجب أن تطابق العملة المعروضة في الصفحة**، وإلا عُدَّت الصفحة مضلِّلة.

**(2) الصفحة الساكنة تحمل الدولار، والليرة تُحقن عند الطلب.** الحل الساذج — إعادة بناء الصفحات عند كل تحديث لسعر الصرف — يعني إعادة بناء آلاف الصفحات لتغيير رقم واحد، عدة مرات شهرياً، وهو هدر لا يُحتمل. القاعدة المعتمدة:

| الطبقة | ما تحمله | متى تتغيّر |
| --- | --- | --- |
| الصفحة الساكنة المولَّدة | السعر بالدولار + `priceCurrency: "USD"` في البيانات المنظمة | فقط عند تغيّر سعر المنتج بالدولار أو مواصفاته |
| طبقة الحافة (Edge) | تحقن مبلغ الليرة وسعر الصرف الساري في وسم `<span data-syp>` وفي البيانات المنظمة قبل تسليم الصفحة | عند كل طلب، من ذاكرة مؤقتة عمرها 60 ثانية |
| الجزيرة التفاعلية | تبديل العملة وإعادة الحساب دون طلب جديد | عند تفاعل المستخدم |

بهذا يبقى الإصدار الساكن مستقراً، ويبقى الرقم المعروض حياً، ولا يُعاد بناء شيء لتغيّر سعر الصرف. أما `priceValidUntil` فيبقى قصيراً (24 ساعة) ويُحقن من الحافة أيضاً. ولا يحمل الكائن أي معلومة عن وسائل الدفع.
