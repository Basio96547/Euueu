...
### 9.11 حدود المعدل للمسارات المضافة في الفصول 14–18

جدول حدود المعدل في هذا الفصل هو المصدر الوحيد للوثيقة كلها، ولا يكتمل إلا بتغطية المسارات التي أضافتها الفصول اللاحقة:

| المسار | الحد | النطاق | السبب |
| --- | --- | --- | --- |
| `POST /tracking/lookup` | 10 / ساعة | IP + رقم الطلب | منع تعداد أرقام الطلبات (enumeration) |
| `GET /warranties/verify` | 20 / ساعة | IP | صفحة عامة تقبل IMEI؛ الحد يمنع مسح قوائم IMEI |
| `POST /support/tickets` | 5 / يوم | مستخدم | منع إغراق الدعم |
| `POST /support/tickets/{n}/messages` | 30 / ساعة | مستخدم | |
| `POST /me/warranty-claims` | 3 / يوم | مستخدم | |
| `POST /catalog/products/{id}/questions` | 5 / يوم | مستخدم | |
| `POST /me/alerts/*` | 20 / يوم | مستخدم | |
| `POST /courier/orders/{id}/collect` | 120 / ساعة | مندوب | سقف تشغيلي معقول يكشف التكرار الآلي |
| `POST /courier/orders/{id}/status` | 300 / ساعة | مندوب | |
| `POST /admin/fx` | 20 / يوم | مدير | تغيير سعر الصرف حدث نادر ومؤثر؛ التكرار المفرط مؤشر خطأ أو عبث |
| `POST /admin/catalog/imports` | 10 / يوم | مسؤول كتالوج | |
| `POST /admin/orders/{id}/confirm` | 600 / ساعة | موظف تأكيد | |

**قاعدة عامة**: أي مسار جديد يُضاف إلى الوثيقة لاحقاً يجب أن يظهر في هذا الجدول قبل تنفيذه، والمسار غير المذكور هنا يرث الحد الافتراضي العام (60 طلباً/دقيقة لكل IP) وهو حد غير مقصود لأي عملية حساسة.

### 9.12 اختبار القناة الاحتياطية

قناة SMS الاحتياطية لا تُعامل بوصفها موجودة ما لم تُختبر: مهمة `maint.fallback-channel-drill` الأسبوعية (القسم 11) ترسل رسالة حقيقية عبرها إلى رقم داخلي وتتحقق من وصولها خلال 30 ثانية، وتُصعِّد فوراً عند الفشل. والتبديل التلقائي من واتساب إلى SMS يُشغَّل عند تجاوز فشل التسليم 5% خلال 15 دقيقة، لا بقرار بشري لحظة الأزمة.
