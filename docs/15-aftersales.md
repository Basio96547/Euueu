...
كل ما يلي `id UUID` بنمط UUIDv7، و`created_at`/`updated_at` من نوع `TIMESTAMPTZ`، والمبالغ بالدولار في `*_usd_cents BIGINT` والمبالغ النقدية المحصّلة بالليرة مقرَّبة لأقرب 10.
...
| `charged_syp` | `BIGINT NULL` | محصّل نقداً، `CHECK (charged_syp % 10 = 0)` |
...
قيمة `intake_cost_bearer` تُحدَّد عند الفتح مبدئياً وتُصحَّح بعد التشخيص، وأي كلفة على العميل تُقيَّد في `repair_jobs.charged_syp` وتُسوّى ضمن `cash_settlements` كما في القسم 8، مقرَّبة لأقرب 10 ليرة ومحسوبة من سعر الدولار المثبّت يوم إصدار عرض الإصلاح وصلاحيته 48 ساعة كسائر الأسعار.
