...
### 17.11 سلامة التحصيل عند المزامنة المتأخرة

واجهة المندوب تعمل دون اتصال، والتحصيل نقدي — وهذان معاً يخلقان ثلاث مخاطر لا يجوز تركها للاجتهاد:

**(1) التحصيل المزدوج.** إعادة إرسال الطلبية المؤجَّلة بعد عودة الشبكة قد تُسجِّل المبلغ مرتين. لذلك كل عملية تحصيل تحمل `Idempotency-Key` يولّده جهاز المندوب من `(order_id, device_id, local_seq)`، ويُخزَّن في Redis 72 ساعة؛ الإرسال المكرر بالمفتاح نفسه يعيد النتيجة الأولى دون أثر جانبي. ولا يُقبل تسجيل تحصيل ثانٍ لطلب حالته `payment_status='COLLECTED'` إلا بصلاحية `finance:adjust:any` ومع سبب مسجَّل في `audit_logs`.

**(2) التباس زمن الحدث بزمن الوصول.** كل سجل يحمل طابعين: `occurred_at` من ساعة جهاز المندوب (وقت التسليم الفعلي)، و`received_at` من الخادم (وقت وصول المزامنة). **التقارير المالية اليومية تُبنى على `occurred_at`**، وإلا ظهر تحصيل الأمس في حصيلة اليوم. وإذا انحرفت ساعة الجهاز أكثر من 10 دقائق عن الخادم يرفض التطبيق التسجيل ويطلب مزامنة الوقت، لأن طابعاً زمنياً خاطئاً يفسد التسوية بصمت.

**(3) بيانات تتبّع تقيس الانضباط لا الواقع.** ما دام التحديث يدوياً، فإن «زمن التسليم» يقيس متى سجّل المندوب لا متى سلّم. لذلك يُقاس **فارق التسجيل** (`received_at − occurred_at`) بوصفه مؤشر جودة بيانات مستقلاً (مهمة `maint.tracking-lag`, القسم 11)، ويُعرض بجانب كل مؤشر زمني في التقارير. الوسيط المستهدف أقل من 15 دقيقة؛ وإذا تجاوز ساعة فكل مؤشرات الزمن في ذلك الأسبوع تُوسم «تقديرية» ولا تُبنى عليها قرارات.

### 17.12 الطلب المُنشأ دون اتصال

طلب أنشأه العميل بلا شبكة ووصل بعد ساعات ليس طلباً عادياً: السعر قد تغيّر، وسعر الصرف قد تقادم، والمخزون قد نفد. سياسة القبول عند المزامنة إلزامية وبهذا الترتيب:

| الفحص | عند الفشل |
| --- | --- |
| صلاحية سعر الصرف المستخدم في التسعير المحلي | يُعاد التسعير بالسعر الساري ويُعرض الفرق على العميل قبل الإنشاء |
| تغيّر سعر المنتج بالدولار | يُرفض الإنشاء الصامت، ويُطلب تأكيد السعر الجديد |
| توفر المخزون | يُقترح بديل أو تُلغى العملية بلا رسوم |
| عمر الطلب المؤجَّل > 12 ساعة | يُرفض ويُطلب إعادة الإنشاء — الطلب القديم جداً بيانات ميتة لا نية شراء |

ويحمل الطلب المؤجَّل `Idempotency-Key` بالقاعدة نفسها حتى لا يتكرر عند المحاولات المتعددة.
