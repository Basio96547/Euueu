...
يغطّي مسار الطلب دورة الحالات المعتمدة في القسم رقم 8 حصراً: `PENDING_CONFIRMATION → PROCESSING → SHIPPED → OUT_FOR_DELIVERY → DELIVERED`، إضافة إلى `DELIVERY_FAILED` و`RETURN_REQUESTED` و`CANCELLED`. ويتحقق اختبار مخصّص من أن إنشاء الطلب يحوّل الحجز المرن إلى حجز مؤكَّد مدته **48 ساعة**، وأن `orders.expire-pending` تلغي الطلب بعد انقضائها أو بعد ثلاث محاولات اتصال فاشلة، وأن سعر الصرف المثبّت على الطلب لا يتغيّر بتغيّر السعر المعروض في الواجهة.
