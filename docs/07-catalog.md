...
**تحويل العرض والتقريب.** التحويل يحدث في طبقة العرض فقط، والتقريب موحّد لأقرب 10 ليرات في كل الشاشات والإيصالات:
...
مسار التسعير ينتهي عند مبلغ مستحق نهائي بالسنتات، ومنه المبلغ النقدي بالليرة المقرَّب لأقرب 10. **إضافة طريقة دفع مستقبلاً = قيمة جديدة في `payment_method` وخطوة إضافية في التدفق، دون إعادة هيكلة.**
...
### 7.10 المنتجات التجريبية والتحويل إلى منتجات حقيقية

فحص المتجر على قاعدة بيانات فارغة لا يكشف شيئاً: لا شكل الشبكة، ولا سلوك المرشحات، ولا أداء الصفحة بصور حقيقية. لذلك يبدأ المشروع ببذرة كتالوج تجريبية (المجلد `seed/`)، ويُدار الانتقال منها إلى الكتالوج الحقيقي بوصفه **مساراً محكوماً** لا تبديلَ علامة.

**العلامة**: `products.is_demo BOOLEAN NOT NULL DEFAULT false` (امتداد لجدول `products` في القسم رقم 3)، ومعها `product_variants.demo_stock SMALLINT NULL` وهي كمية **عرضية بحتة**: لا تمر بدفتر `inventory_movements`، ولا تُنشئ صفوفاً في `device_units`، ولا تدخل في `inventory_levels`. أي أن المنتج التجريبي لا يملك مخزوناً حقيقياً بالمعنى الذي يعرفه النظام.

**السلوك المفروض** — العلامة تغيّر النظام لا الواجهة فقط:

| المحور | المنتج التجريبي |
| --- | --- |
| الظهور | بيئات التطوير والتجريب فقط؛ وفي الإنتاج يظهر حصراً عند تفعيل `DEMO_MODE` في إعدادات المتجر |
| فهرس البحث وخريطة الموقع | مستبعَد شرطاً في وقت الفهرسة (`is_demo = false` جزء من استعلام البناء)، فلا يتسرب إلى جوجل |
| السلة والطلب | إضافته للسلة ممكنة في بيئة التجريب، وإنشاء الطلب مرفوض برمز `DEMO_PRODUCT_NOT_ORDERABLE` (القسم رقم 4) |
| الحجز | لا يُنشئ `inventory_reservations` إطلاقاً |
| بوابة الإقلاع | التطبيق **يرفض الإقلاع في الإنتاج** إن وُجد منتج `is_demo = true` بحالة `PUBLISHED` و`DEMO_MODE` معطَّل — فشل صريح خير من متجر يعرض بضاعة لا وجود لها |

**مسار التحويل** من لوحة التحكم (زر «تحويل إلى منتج حقيقي»)، ولا يكتمل إلا بعبور كل بوابة:

```mermaid
stateDiagram-v2
    [*] --> DEMO
    DEMO --> CONVERTING: start conversion
    CONVERTING --> DEMO: validation failed
    CONVERTING --> DRAFT: cost + receipt + media + IMEI ok
    DRAFT --> IN_REVIEW: submit (7.2 gates)
    IN_REVIEW --> PUBLISHED: approve
```

| البوابة | الشرط |
| --- | --- |
| التكلفة | `cost_price_usd_cents` حقيقي لكل متغيّر (قيمة البذر التجريبية تُمحى ولا تُورَّث) |
| المخزون | إدخال الكمية عبر **إيصال استلام فعلي** (القسم رقم 16) يكتب في `inventory_movements` بسبب `RECEIPT`؛ و`demo_stock` يُصفَّر ويُهمَل |
| الأجهزة | أرقام IMEI مدخلة لكل وحدة من فئة `phones`، بفحص Luhn وفرادة عالمية |
| الوسائط | صور حقيقية تحل محل العنصر النائب؛ ولغير الجديد ست صور للجهاز نفسه بـ `is_real_unit_photo = true` |
| فروق السوق | قيم مؤكَّدة لـ `device_origin` و`part_code` و`condition` و`battery_health_pct` و`warranty_type` و`warranty_months` — لا قيم موروثة من البذرة بلا مراجعة |
| السعر | تأكيد صريح للسعر بالدولار بعد رؤية التكلفة والهامش المحسوب |

بعد التحويل يعود المنتج إلى `DRAFT` ويمر بدورة الحياة المعتادة في 7.2 كأي منتج جديد؛ لا يوجد طريق مختصر ينقل منتجاً تجريبياً إلى `PUBLISHED` مباشرة. وكل تحويل يُسجَّل في `audit_logs` بالفاعل والقيم قبل وبعد.

**التنظيف**: إجراء `db:purge-demo` يحذف كل ما هو `is_demo` دفعةً واحدة، ويفشل إن كان أي منتج تجريبي مرتبطاً بطلب أو مطالبة كفالة — وهي حالة لا ينبغي أن تقع أصلاً لأن الطلب على المنتج التجريبي مرفوض، فوقوعها بحد ذاته يعني خللاً يستحق التوقف عنده. ومن بنود فحص الجاهزية قبل الإطلاق (القسم رقم 19): **صفر منتجات تجريبية منشورة**.

### 7.11 انتشار سعر الصرف إلى كل الأسعار

المدير يغيّر رقماً واحداً في شاشة `/fx-rates` (القسم رقم 10)، فتتغيّر أسعار الكتالوج كله بالليرة. هذا القسم يحدد **كيف** يصل ذلك الرقم إلى كل سطح، وكيف لا يُعاد بناء الموقع لأجله، وأين يُقرَّب بالضبط.

**المعادلة المرجعية**

```
السعر بالليرة (قبل التقريب) = (price_usd_cents × rate) ÷ 100
السعر المعروض              = round(السعر بالليرة ÷ 1000) × 1000
```

حيث `rate` عدد الليرات لكل دولار واحد، مأخوذاً من الصف الساري في `fx_rates` (أحدث صف بـ `effective_from <= now() < valid_until`). هذه المعادلة **وحيدة**: مُنفَّذة في `packages/ui/src/money.ts` ومستوردة في التطبيقات الثلاثة، وممنوع إعادة كتابتها في أي مكان آخر.

**مسار الانتشار — من الشاشة إلى كل سطح**

| السطح | ما يصله | كيف |
|---|---|---|
| شاشة `/fx-rates` | إدخال السعر و`effective_from` و`valid_until` | صف جديد في `fx_rates`، ولا تعديل على صف قديم أبداً |
| ذاكرة الحافة | `fx_version` جديد يُبطل الذاكرة فوراً | نشر الحدث `fx.updated` على BullMQ ⇒ تفريغ مفتاح `fx:current` في Redis وذاكرة الحافة |
| صفحات Astro الساكنة | مبلغ الليرة يُحقن عند الطلب في `<span data-syp>` وفي البيانات المنظمة | حقن على الحافة من `fx:current` بذاكرة 60 ثانية — **بلا إعادة بناء للصفحة** |
| نتائج البحث | السعر بالدولار من الفهرس، والليرة تُحسب في المتصفح | فهرس Meilisearch لا يحوي ليرة إطلاقاً (القسم رقم 6) |
| بطاقة المنتج وصفحته | القيمتان معاً، والمختارة أكبر خطاً | مفتاح تبديل العملة في الرأس + كوكي `currency_pref` |
| السلة و`/app` | `usd_cents` + `rate` + `fx_version` في كل استجابة | إعادة حساب فورية في العميل، وتحذير إن تغيّر `fx_version` أثناء الجلسة |
| الطلب المُنشأ | **لا شيء** — `orders.fx_rate` و`total_syp` مثبَّتان | تغيير السعر لا يمسّ طلباً قائماً داخل نافذة 48 ساعة |
| الإيصال والفاتورة | السعر المثبَّت على الطلب لا سعر اليوم | إعادة طباعة فاتورة قديمة تعطي الرقم نفسه دائماً |
| التقارير | سعر الصرف المثبَّت على كل طلب | تقارير الماضي لا تتغيّر كلما تحرّك السعر |

**زمن الانتشار المستهدف**: أقل من 60 ثانية من اعتماد السعر حتى ظهوره في كل السطوح الحية، وصفر إعادة بناء.

**قاعدة التقريب: يُقرَّب مرة واحدة في النهاية، ولا تُجمع أرقام مقرَّبة**

هذه القاعدة تحسم خللاً حسابياً حقيقياً: لو قُرِّب سعر كل صنف على حدة ثم جُمعت الأرقام المقرَّبة، لاختلف المجموع عن المبلغ المستحق فعلاً، ولاتّهم العميل المتجر بالتلاعب وهو محقّ في شكّه.

| المستوى | التقريب |
|---|---|
| سعر الصنف على البطاقة والصفحة | يُقرَّب لأقرب 10 **للعرض فقط**، وقيمة غير ملزِمة |
| مجموع السلة والطلب | يُحسب من **مجموع السنتات الدولارية** ثم يُقرَّب **مرة واحدة** |
| المبلغ المستحق نقداً | هو المجموع المقرَّب مرة واحدة، وهو وحده الملزِم |

مثال بثلاثة أصناف وسعر صرف `12850`:

| الصنف | سنتات | ليرة خام | معروض على البطاقة |
|---|---|---|---|
| Galaxy A16 | 14500 | 1,863,250 | 1,863,000 |
| شاحن 33W | 1200 | 154,200 | 154,000 |
| كابل | 700 | 89,950 | 90,000 |
| **جمع المعروض (خاطئ)** | — | — | **2,107,000** |
| **الطريقة المعتمدة** | 16400 | 2,107,400 | **2,107,000** |

تصادف الرقمان هنا، لكنهما يفترقان كثيراً بأصناف أخرى — ولهذا يعرض تفصيل السلة سطراً صريحاً: «المبلغ المستحق يُحسب على المجموع، وقد يختلف عن جمع الأسعار المعروضة بمقدار أقل من 10 ليرات»، وفرق التقريب يُخزَّن في `orders.rounding_diff_syp` (القسم رقم 8).

**معاينة الأثر قبل الاعتماد** (شاشة `/fx-rates`): قبل تثبيت أي سعر جديد تعرض اللوحة نسبة الانحراف عن السعر الساري، وعدد المنتجات التي سيتغيّر سعرها المعروض بأكثر من 10 ليرات، وأغلى عشرة منتجات بسعريها القديم والجديد، وعدد الطلبات المحمية بالتثبيت التي **لن** تتأثر. والانحراف الحاد يستوجب تأكيداً ثانياً وملاحظة إلزامية، ويفتح تنبيهاً بمراجعة الكوبونات ذات المبالغ الثابتة وقواعد التسعير النشطة — لأن خصماً بمبلغ ثابت بالدولار يتصرف تصرفاً مختلفاً تماماً بعد قفزة في سعر الصرف.

**الحارس**: إن انقضى `valid_until` بلا تحديث، لا يستمر النظام بالسعر القديم صامتاً — بل يتدرّج من التنبيه إلى هامش الأمان إلى إيقاف استقبال الطلبات وفق الجدول في القسم رقم 3.
