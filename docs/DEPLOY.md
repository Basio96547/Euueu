...
ومسار `/api/v1/bootstrap` حُذف: كان يضبط أوّل كلمة سرّ لحساب الإدارة بلا
رمز ولا مصادقة، وحارسُه الوحيد أن الحقل ما زال فارغاً — وذلك سباقٌ لا إذن.
وضبطُها الآن بـ`bash scripts/set-admin-password.sh +9639XXXXXXXX`، يمرّ
بمفتاح Cloudflare: من يملكه يملك القاعدة أصلاً، فهو الحدّ الصحيح للثقة.
...
والختم قبل الترحيل: القاعدة الحيّة هُيِّئت بمسارٍ داخليّ لا بـwrangler،
