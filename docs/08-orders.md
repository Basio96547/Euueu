...
sequenceDiagram
  ...
  API->>INV: firm_reserve(orderId, ttl=48h) [ORDER_HOLD]
...
| الحجز المؤكَّد | عند إنشاء الطلب يتحول إلى `ORDER_HOLD` مدته **48 ساعة** مرتبطة بنافذة التأكيد الهاتفي |
...
| البند | القاعدة |
|---|---|
| عدد المحاولات | 3 كحد أقصى (`orders.confirmation_attempts`، قيد `CHECK (confirmation_attempts <= 3)`) |
| تباعد المحاولات | الأولى خلال نصف ساعة من إنشاء الطلب، الثانية بعد 4 ساعات، الثالثة صباح اليوم التالي — كلها داخل نافذة 09:00–21:00 بتوقيت `Asia/Damascus` |
| نافذة التأكيد | **48 ساعة** من `placed_at`، وهي نفسها عمر `ORDER_HOLD` وعمر `price_locked_until` |
| تسجيل النتيجة | `confirmed_by`, `confirmed_at`, `confirmation_notes`, `confirmation_attempts` + صف في `order_status_history` |
| الإلغاء التلقائي | مهمة `orders.expire-pending` تنقل إلى `CANCELLED` بعد **48 ساعة** أو بعد **3 محاولات اتصال فاشلة**، أيهما أسبق، وتحرّر `ORDER_HOLD` فوراً |
| قناة بديلة | إن تعذّرت المكالمة تُرسل رسالة واتساب بزرَّي «أؤكد الطلب» / «ألغِ الطلب»؛ ضغط زر التأكيد يعادل مكالمة ناجحة ويُسجَّل بـ `confirmation_notes='WHATSAPP_SELF_CONFIRM'` |

نتائج المكالمة المقبولة في `POST /admin/orders/{id}/confirm` عبر الحقل `outcome`:

| القيمة | الأثر على الحالة | الأثر على الحجز |
|---|---|---|
| `CONFIRMED` | `PENDING_CONFIRMATION → PROCESSING` | يبقى `ORDER_HOLD` حتى التجهيز |
| `NO_ANSWER` | تبقى الحالة، `confirmation_attempts += 1` | يبقى الحجز حتى انتهاء الـ48 ساعة |
| `RESCHEDULE` | تبقى الحالة مع `next_attempt_at` | يبقى الحجز |
| `ADDRESS_FIXED` | تبقى الحالة، يُحدَّث العنوان ويُعاد حساب الشحن | يبقى الحجز |
...
| --- | --- | --- |
| `→ PENDING_CONFIRMATION` | العميل | `POST /orders` مع `Idempotency-Key`؛ حجز مؤكَّد 48 ساعة |
...
مثال الطلب `TS-2607-000148`: الإجمالي `90200` سنتاً (902.00 دولار) وسعر الصرف المثبَّت `12850` ⇒ خام `11,590,700` ليرة ⇒ **المستحق نقداً `11,591,000` ليرة**. فرق التقريب (≤ 500 ليرة) لا يُخزَّن ولا يُطالَب به ولا يُعاد؛ لأن الفئات النقدية الصغيرة غير متوفرة عملياً. الرقم المقرَّب هو **نفسه** على: إيصال PDF المرفق مع الشحنة، وشاشة المندوب، ورسالة واتساب قبل الخروج للتوصيل، وبوليصة مكتب النقل.
