...
### 16.10 حساب فروق التقريب

تقريب المبلغ النقدي لأقرب 10 ليرات (القسم 8) يولّد فرقاً صغيراً في كل طلب: ±5 ليرات كحد أقصى، لا يُطالَب به ولا يُعاد. هذا الفرق صغير على مستوى الطلب لكنه **لا يختفي**: على ألف طلب شهرياً يصير مبلغاً ظاهراً في الحسابات، ولو تُرك بلا قيد لظهر عجزاً غامضاً في التسوية بينما هو أثر حسابي معروف السبب.

لذلك يُخزَّن الفرق في `orders.rounding_diff_syp` ويُرحَّل شهرياً إلى بند مستقل:

| البند | الحساب | المعالجة |
| --- | --- | --- |
| فروق التقريب | `SUM(orders.rounding_diff_syp)` للفترة | بند مستقل في قائمة الأرباح ضمن «إيرادات/مصاريف أخرى» |
| مطابقة التسوية | `SUM(collected_amount_syp) − SUM(total_syp)` | يجب أن يساوي **صفراً** بعد استبعاد فروق التقريب؛ أي باقٍ بعد ذلك عجز حقيقي يُحقَّق فيه |

**قاعدة رقابية**: صافي فروق التقريب يجب أن يقترب من الصفر مع كثرة الطلبات (توزيع متماثل حول الصفر). انحيازه المستمر في اتجاه واحد يعني خللاً في دالة التقريب أو تلاعباً في المبالغ، وكلاهما يستوجب تحقيقاً لا تفسيراً. ويُراقَب كذلك حجم الفرق نسبةً إلى متوسط قيمة الطلب: إذا صارت وحدة التقريب (10 ليرات) نسبة معتبرة من قيمة الطلب فذلك مؤشر على تغيّر قيمة العملة ووجوب مراجعة الوحدة نفسها.
