...
export class Amygdala implements Lobe<AmygdalaState> {
  ...
  constructor(rng?: Rng) {
    ...
    /* وتُصفَّر أوزانها بعد التهيئة، خلافاً لكل فص آخر.
     *
     * تهيئة Xavier عشوائية، فلوزةٌ لم تُشترَط قط تُخرج وسماً عشوائياً في
     * ‎[-0.6, +0.6]‎: يولد زبير كارهاً لكلمة ومحبّاً لأخرى بلا أن يجرّب أياً
     * منهما. ولم يظهر هذا إلا بعد بناء المشاعر، إذ صار الوسم يُقرأ حزناً وخوفاً
     * فبدا الوليد حزيناً قبل أن يُكلَّم — والصواب أن مَن لا تجربة له لا وسم عنده.
     *
     * والتصفير لا يمنع التعلّم هنا لأن الطبقة واحدة: تدرّج الوزن هو المدخل
     * مضروباً في الخطأ، ولا يصير صفراً إلا بمدخل صفر. (وهذا لا يصلح لشبكة ذات
     * طبقات، إذ تتماثل خلاياها فلا تتفرّق أبداً.) */
    this.net.w.fill(0);
  }
