...
export class Syntax implements Lobe<SyntaxState> {
  ...
  request(percept: Percept, parse: Parse): Request {
    ...
    /* ٠. المخرج الثالث: أعلامةٌ واحدة أم علامتان متنازعتان؟
     *
     * المالكُ الحتميّ يصير مُخطئاً حتمياً على المسكوكات، وهذا أثبتُّه بالقياس
     * لا بالظنّ: «كيف بتعمل هالشي؟» خرجت «صفة» وهي تطلب فعلاً، و«مين قدك»
     * خرجت «شخص» وهي ليست سؤالاً. فالعلامةُ حين تنفرد قاطعة، وحين تُنازِعها
     * أخرى فالصدق أن يُقال: لم أحدّد.
     *
     * ولا يُخمَّن الالتباس: يُرصَد بثلاث علاماتٍ ظاهرة يُحصى كلٌّ منها. */
    if (this.ambiguous(percept, parse)) return 'مُلتبس';
  ...
  /**
   * أفشل الاستخراج؟ ثلاث علاماتٍ تُرصَد ولا تُخمَّن:
   *
   *  ١) **علامتان متنازعتان**: أداةُ صفةٍ وفعلُ استخبارٍ في جملةٍ واحدة —
   *     «كيف بتعمل هالشي؟» صفةٌ أم فعل؟ اثنتان تصلحان، فلا واحدةَ تُختار.
   *
   *  ٢) **سؤالٌ بلا موضوع**: أداةُ استفهامٍ ولا كلمةَ معنى بعدها يُسأل عنها —
   *     «شو هالحكي» و«مين قدك»: الأداة حاضرة والمسؤولُ عنه غائب. ومن سأل عن
   *     لا شيء لم يسأل.
   *
   *  ٣) **أداةٌ في وسط الجملة بعد شرط**: «وين ما تروح» و«كيف ما كان» — «ما»
   *     بعد أداة الاستفهام تقلبها شرطاً لا سؤالاً، وهذه مسكوكةٌ لا استفهام.
   */
  private ambiguous(percept: Percept, parse: Parse): boolean {
    const tokens = percept.tokens;
    /* وعلامةٌ قاطعةٌ تُنهي الالتباس قبل أن يبدأ: «كيفك» سؤالٌ عن حاله بلا
     * ريب، و«شو رأيك» طلبُ رأيٍ بلا ريب. والالتباس يُرصَد حيث لا قاطع. */
    if (tokens.some((t) => ABOUT_SELF.has(t) || OPINION.has(t))) return false;
    const hasTool = tokens.some((t) => QUESTION_TOOLS.has(t));

    /* (٣) أداةٌ حاضرة ولم يخرج للسؤال نوع: «وين ما تروح» و«كيف ما كان» —
     * «ما» بعد الأداة تقلبها شرطاً، فتسقط دلالة الاستفهام ويبقى اللفظ. ومن
     * قرأها خبراً أخطأ كمن قرأها سؤالاً، فالصدق أن يستوضح. */
    if (parse.asks === null) {
      return hasTool && parse.topic === null && !parse.corrects;
    }

    for (let i = 0; i + 1 < tokens.length; i++) {
      if (QUESTION_TOOLS.has(tokens[i]!) && tokens[i + 1] === 'ما') return true;
    }

    // (١) أداةُ صفةٍ مع فعل استخبار
    if (parse.asks === 'كيفية' && tokens.some((t) => DOING.has(t))) return true;

    // (٢) أداةٌ بلا موضوعٍ يُسأل عنه
    const content = parse.words.filter((w) =>
      w.pos !== 'حرف'
      && !QUESTION_TOOLS.has(w.word)
      && !NEGATIONS.has(w.word)
      && !PRONOUNS.has(w.word)
      && !DOING.has(w.word)
      && !SOCIAL_WORDS.has(w.word)
      /* و«هالـ» إشارةٌ إلى حاضرٍ لا يراه: «شو هالحكي» تُشير إلى شيءٍ في الغرفة،
       * وزبير لا غرفةَ له. فالمشارُ إليه غائبٌ عنه، ومن سُئل عن غائبٍ لم يُسأل. */
      && !/^هال./.test(w.word)
      && w.stem.length >= 3);
  ...
  storeFor(request: Request): RelationKind | null {
    switch (request) {
      ...
      case 'حال':
      /* والملتبس لا يُبحث له عن جوابٍ أصلاً: جوابه سؤال */
      case 'مُلتبس':
        return null;
    }
  }
