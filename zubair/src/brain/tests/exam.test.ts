...
/* ————— الرتب: الدعوى التي حلّت محلّ الرقم —————
 *
 * الرقم لا يفصل صوابه عن خطئه، فحلّت الرتبة محلّه في المراجعة. ودعواها
 * واحدة: **أعلاها أصوبُ من أدناها**. فإن انكسر ترتيبُها فهي كالرقم — اسمٌ
 * جديد لعدم تمييز. */
test('ورتبُه لم ينكسر ترتيبُها', async () => {
  const report = await once();
  assert.equal(report.ranksOrdered, baseline.ranksOrdered);
});
